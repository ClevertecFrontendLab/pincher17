...
import { ButtonAddExercise, ButtonCreateExercise, LineExercise, LineExercise2, LineTopExercise, ModalCreateExercise, PopoverStyled, TitlePopover, WrapperButtonPopover, WrapperContentPopover, WrapperPopover, WrapperSelectPopover } from '@pages/calendar/calendar.styles';
import { ArrowLeftOutlined, CloseOutlined, EditOutlined } from '@ant-design/icons';
import { SidePanel } from '@components/SidePanel';
import { DateCellWithTrainings, EmptyExercise, ExerciseName, WrapperAllCalendar, WrapperCalendar, WrapperDateCell, WrapperExercises, WrapperMobileCalendar } from './CalendarExercises.styles';
import { useAppDispatch, useAppSelector } from '@hooks/typed-react-redux-hooks';
import { TrainingsType, changeTrainingThunk, createTrainingThunk, setSuccessTrainingAdd } from '@redux/trainingSlice';
...
function convertDateFormat(dateString: string | number): string {
  ...
  if (typeof dateString === 'number') {
    ...
    datePart = `${dateObject.getFullYear()}-${(dateObject.getMonth() + 1).toString().padStart(2, '0')}-${dateObject.getDate().toString().padStart(2, '0')}`;
  } else {
...
export const CalendarExercises: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if(!isSelectedDatePastOrToday){
        const todayTrainings = trainings.filter((item) => {
          return convertDateFormat(item.date) === convertDateFormat(selectedDateISOFormat);
        }).map((item) => item.name);
        /* setTodayTrainings(todayTrainings) */
        const filteredTrainingsList = trainingsListAll.filter(item => !todayTrainings.includes(item.value));
  ...
  useEffect(() => {
    ...
    const selectedObject = trainings.find(
      (item: any) => convertDateFormat(item.date) === convertDateFormat(selectedDateISOFormat) && item.name === selectedExercise
    );
  ...
  const saveTrainings = () => {
    ...
    if (exercisesDrawer._id) {
      dispatch(changeTrainingThunk(accessToken, exercisesDrawer._id, updatedExercisesDrawer2));
    } else {
  ...
  const contentPopover = (<WrapperPopover data-test-id='modal-create-training'> 
                              <WrapperContentPopover>
                              <TitlePopover>Тренировки на {selectedDate}</TitlePopover>
                              <div>
                                <CloseOutlined data-test-id='modal-create-training-button-close' onClick={hidePopover} size={16} />
                              </div>
                              ...
                              <WrapperExercises>
                                    {  todayTrainings.map((item, index, ) => (
                                          ...
                                          ? <Button 
                                                ...
                                                >
                                            <Badge color={getColor(item.name)} style={{color: 'inherit'}} text={item.name} />
                                            <EditOutlined style={{color: item.isImplementation ? 'rgba(140, 140, 140, 1)' : 'rgba(24, 144, 255, 1)', cursor: 'pointer'}} />
                                            </Button>
                                          : ''
                                          ))}
                                    </WrapperExercises>
...
const contentPopoverExercise = (<WrapperPopover data-test-id='modal-create-exercise'>
                            <div>
                            <WrapperSelectPopover>
                            <div style={{marginRight: '5px'}}>
                              <ArrowLeftOutlined data-test-id='modal-exercise-training-button-close' 
                                  style={{fontSize: '18px', cursor: 'pointer'}} onClick={hidePopoverExercise}  />
                            </div>
                            <Select
                            ...
                            <WrapperExercises>
                              ...
                              ? exercisesDrawer?.exercises?.map((i: any, index: any) => (
                                <ExerciseName data-test-id={`modal-update-training-edit-button${index}`} onClick={showDrawer}>
                                  <div>{i.name}</div>
                                  <EditOutlined style={{color: 'rgba(24, 144, 255, 1)', cursor: 'pointer'}}  />
                                </ExerciseName>
                            ...
                            <ButtonAddExercise
                                  ...
                                  <ButtonAddExercise
                                      disabled={exercisesDrawer.exercises.length === 0 && exercisesDrawer.userId === ''}
                                      type='link'
  ...
  const handleCellClick = (value: any) => {
    if (!panelChangedRef.current) {
          ...
          const dateString = `${value.date().toString().padStart(2, '0')}.${(value.month() + 1).toString().padStart(2, '0')}.${value.year()}`;
          const dayOfWeek = value.day();3
          if(dayOfWeek === 0){
            setSunday(true)
          }else{
            setSunday(false)
            ...
            return (
              <>
                ...
                { selectedDate === dateString && resolution.width > 400 &&
                  ...
                  <div style={{ position: sunday || (saturday &&  resolution.width < 1126) ? 'absolute' : 'relative', 
                                top: sunday || (saturday &&  resolution.width < 1126) ? `-20px`: '-50px', 
                                right: sunday || (saturday &&  resolution.width < 1126) ? `-2px` : ``,
                    ...
                    <PopoverStyled
                      ...
                      showArrow={false}
                      placement={sunday || (saturday &&  resolution.width < 1126) ? 'bottomRight' : 'bottom'}
  ...
  const dateCellRenderMobile = (value: any) => {
    ...
    let matchFound = false;
            return (
              <>
                 { selectedDate === dateString && resolution.width <= 400 && <WrapperDateCell onClick={(e) => e.stopPropagation()}>
                 {popoverExercise &&
                    <PopoverStyled
                      content={contentPopoverExercise}
                      trigger="click"
  ...
  return (
  <>
  <SidePanel resolution={resolution} setExercisesDrawer={setExercisesDrawer} exercisesDrawer={exercisesDrawer} openDrawer={openDrawer} setOpenDrawer={setOpenDrawer} selectedDate={selectedDate} selectedExercise={selectedExercise} />
