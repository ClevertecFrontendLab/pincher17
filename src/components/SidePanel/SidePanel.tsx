...
import { Count, CountSets, Date, ExerciseDateWrapper, ExerciseTitle, NameSettings, WeightAndCount, WrapperButton, WrapperCreateExercise, WrapperSettingsExercise, WrapperSidePanel, Xicon } from './SidePanel.styles';
...
export const SidePanel: React.FC<SidePanelProps> = ({setOpenDrawer, openDrawer, selectedDate, 
                        selectedExercise, exercisesDrawer, setExercisesDrawer, resolution }) => {
  ...
  const deleteCheckedExercises = () => {
    if(isSelectedDatePastOrToday){
    setExercisesDrawer((prevExercisesDrawer: any) => {
      const filteredExercises = prevExercisesDrawer.exercises.filter((exercise: any) => !exercise.checked);
      return {
        ...prevExercisesDrawer,
        isImplementation: true,
  ...
  return (
    <>
      <Drawer 
              ...
              onClose={onCloseDrawer} open={openDrawer} 
              width={408}
        ...
        <WrapperSidePanel>
          <ExerciseDateWrapper>
            <ExerciseTitle><Badge color={getColor(selectedExercise)} text={selectedExercise} /></ExerciseTitle>
          ...
          {exercisesDrawer?.exercises?.map((i: any, index: any) => (
            <WrapperCreateExercise key={index}>
              ...
              data-test-id={`modal-drawer-right-input-exercise${index}`}
                ...
                addonAfter={
                  ...
                  data-test-id={`modal-drawer-right-checkbox-exercise${index}`}
                    checked={i.checked}
                    onChange={(e) => {
                      const updatedExercise = { ...i, checked: e.target.checked };
              ...
              <WrapperSettingsExercise>
                <CountSets>
                  ...
                  addonBefore={'+'} min={1} placeholder='1' style={{ width: '100%' }} value={i.replays}
                  onChange={(e) => {
           ...
           <WrapperCreateExercise>
          <Input data-test-id={`modal-drawer-right-input-exercise${indexLastinput}`} placeholder="Упражнение" value={newExercise.name} onChange={(e) => setNewExercise({ ...newExercise, name: e.target.value })} />
            <WrapperSettingsExercise>
              <CountSets>
                <NameSettings>Подходы</NameSettings>
                <InputNumber data-test-id={`modal-drawer-right-input-approach${indexLastinput}`} addonBefore={'+'} min={1} placeholder='1' onChange={onChangeCountSets} style={{ width: '100%' }} value={newExercise.replays}  />
              </CountSets>
              <WeightAndCount>
                <div style={{ marginRight: '14px' }}>
                  <NameSettings>Вес, кг</NameSettings>
                  <InputNumber data-test-id={`modal-drawer-right-input-weight${indexLastinput}`} min={0} placeholder='0' onChange={onChangeWeight} value={newExercise.weight} />
                </div>
                <Xicon>X</Xicon>
                <div>
                  <NameSettings>Количество</NameSettings>
                  <InputNumber data-test-id={`modal-drawer-right-input-quantity${indexLastinput}`} min={1} placeholder='3' onChange={onChangeCount} value={newExercise.approaches}  />
                </div>
              </WeightAndCount>
            </WrapperSettingsExercise>
            <WrapperButton>
              <Button type="link" icon={<PlusOutlined color='rgba(24, 144, 255, 1)' />} onClick={addExercise}>
                Добавить ещё
              </Button>
              <Button disabled={!(exercisesDrawer.exercises.some((item: any) => item.checked))} type="text" icon={<MinusOutlined color='#000000' />} onClick={deleteCheckedExercises}>
                Удалить
              </Button>
            </WrapperButton>
          </WrapperCreateExercise>
